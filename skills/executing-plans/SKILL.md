...
1. 从头到尾完整阅读计划文档
2. 若 `task-decomposition` 已为此计划生成 WBS，则加载它——WBS 提供了预分析的依赖关系、估算和并行化集群
3. 识别所有实施步骤并统计数量
4. 梳理步骤间的依赖关系（哪些必须先完成）
5. 记录任何歧义或悬而未决的问题
6. 在继续前与用户确认理解无误
...
| 条件             | 阈值                                          |
| ---------------- | --------------------------------------------- |
| 计划中的独立任务 | 包含 3 个及以上可独立实现的任务且依赖关系较少 |
...
| `task-decomposition`             | 上游——提供批次创建的 WBS          | 当计划需要层级化拆解后再创建批次时 |
