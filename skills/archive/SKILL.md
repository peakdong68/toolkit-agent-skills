...
将已完成的功能目录从 `docs/changes/` 归档至 `docs/archive/`，整体移动保留完整子目录结构。通过 git 历史（而非代码分析）验证任务完成情况。此技能与 `code-review` 不同：它检查文档复选框和 git 日志以确认已完成的工作，绝不检查源代码。每次归档运行都会更新全局规范索引 `docs/global/index.md`。
...
1. 使用 Glob 模式匹配 `docs/changes/` 中的所有功能目录：

   ```
   Glob: docs/changes/*/
   ```

   目录命名格式：`<date>_<topic>`（如 `2026-05-10_color-extraction`）。

2. 对于每个功能目录，检查任务完成状态：
   - 读取 `plan.md` 文件，检查 `[x]` / `[ ]` 复选框状态
   - 识别所有任务均标记为 `[x]` 的目录

3. 若目录中有 `prd.md`（PRD 无复选框），通过 git 历史判断完成情况：
   - 检查是否有引用该功能名称或对应主题的提交记录
...
| -------------------------------------------------- | ---------- | ---------- | ------ |
| docs/changes/2026-05-10_login/plan.md                     | 5/5 [x]    | 3 条匹配   | 高     |
```
...
**目标：** 将已完成的功能目录整体移至 `docs/archive/` 并清理。

### 操作步骤

1. 确保 `docs/archive/` 存在：
...
2. 将整个功能目录移入归档区：
   ...
   目录内所有产物（`plan.md`、`design.md`、`intent.md`、`prd.md`、`specs/` 等）随整体移动一并归档，结构不变。

3. 验证归档内容完整性 — 使用 Glob 确认目录结构：
   ...
   预期包含：`plan.md` `design.md` `intent.md` `prd.md` `specs/*.md`

4. **修复归档文档中的相对路径引用** — 扫描 `docs/archive/<date>_<topic>/` 中所有 `.md` 文件的内部相对链接。归档后的路径从 `../../changes/` 变为 `../../archive/`，兄弟目录相对路径不变。更新指向仍在 `docs/changes/` 中未归档资源的路径。

5. 确认原始位置已清理 — 使用 folder_operations 检查目录是否还存在：
   ```
...
- [ ] 归档文档中的相对路径引用已检查并修复
- [ ] 原始位置已清理（`docs/changes/<date>_<topic>` 不再存在）
- [ ] 归档目录中包含所有预期文件
...
```
| [<目录名称>](../archive/2026-05-10_color-extraction/plan.md ) | <一句话摘要> | <领域> | <今日 ISO 日期> |
```
...
5. **确保 CLAUDE.md 引用 `docs/global/index.md`** — 如果 CLAUDE.md 的 `## 文档索引` 部分中 `## 核心索引` 表格未引用 `docs/global/index.md`，添加之：
   ```markdown
...
├── plan.md          # planning 技能产物
├── design.md        # brainstorming 技能产物
├── intent.md        # interview-me 技能产物
├── prd.md           # prd-generation 技能产物
└── specs/           # spec-writing 技能产物
    ├── 01-xxx.md
    ├── 02-xxx.md
    └── ...
```
...
| 只移动部分文件（拆散目录归档） | 保留完整功能上下文                 | 整体 mv `<date>_<topic>` 目录，不得拆散 |
| 功能实现前归档                 | 产物仍与活跃工作相关               | 仅在 plan.md 任务全部 `[x]` 后才归档    |
...
- **切勿** 拆散功能目录 —— 始终以整体 `<date>_<topic>` 目录为单位归档
...
| `brainstorming`                  | 设计文档（`design.md`）在归档时随功能目录一并归档            |
| `interview-me`                   | 意图文档（`intent.md`）在归档时随功能目录一并归档            |
| `prd-generation`                 | PRD 文档（`prd.md`）在归档时随功能目录一并归档               |
...
```
用户：/archive
→ 阶段 1 发现 2026-05-10_color-extraction（6/6 [x]，4 条匹配提交 → 高可信度）
→ 阶段 2 展示候选，用户批准
→ 阶段 3 mv docs/changes/2026-05-10_color-extraction docs/archive/
...
**严格（RIGID）** — 五个阶段是顺序且强制性的。Git 历史交叉验证、用户批准关卡和全局索引更新必须严格遵循。绝不检查源代码。整体目录移动，绝不拆散归档。
