...
| `executing-plans` | 上游 —— 计划执行可能需要重试 | 计划执行失败时 |
| `subagent-driven-development` | 上游 —— 子代理任务可能失败 | 子代理任务执行失败时 |
| `autonomous-loop` | 上游 —— 循环迭代可能遇到错误 | 自主循环遇到错误时 |
| `systematic-debugging` | 互补 —— 调试方法论用于失败分析 | 诊断失败根本原因时 |
